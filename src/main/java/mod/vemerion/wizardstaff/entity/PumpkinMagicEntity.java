	...
	private void spawnParticles() {
		...
		for (int i = -1; i < 2; i += 2) {
			...
			for (int j = 0; j < 50; j++) {
				...
				serverWorld.spawnParticle(ModParticles.MAGIC_SMOKE_PARTICLE, pos.x + offset.x, pos.y + offset.y,
		...
		for (int i = 0; i < 50; i++) {
			serverWorld.spawnParticle(ModParticles.MAGIC_SMOKE_PARTICLE, pos.x, pos.y + random.nextDouble() - 0.5,
					pos.z, 1, 0, 0, 0, 0);
