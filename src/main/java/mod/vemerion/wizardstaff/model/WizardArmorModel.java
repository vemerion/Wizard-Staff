...
public class WizardArmorModel<T extends LivingEntity> extends MagicArmorModel<T> {
    ...
    // FIXME: this is no longer called in BipedArmorLayer, so the animations are not working
    @Override
    public void setLivingAnimations(T entityIn, float limbSwing, float limbSwingAmount, float partialTick) {
    	super.setLivingAnimations(entityIn, limbSwing, limbSwingAmount, partialTick);
    	System.out.println(limbSwingAmount);
    	dressBack.rotateAngleX = (float) Math.max(Math.toRadians(20), limbSwingAmount * 1.4);
    	dressFront.rotateAngleX = (float) Math.min(Math.toRadians(-20), -limbSwingAmount * 1.4);
    }
